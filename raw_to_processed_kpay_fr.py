...
# S3 path to the .whl file
s3_path = 's3://acoe-silver-layer/python_libaries/openpyxl-3.1.5-py2.py3-none-any.whl'

# Local path to download the .whl file
local_path = '/tmp/openpyxl-3.1.5-py2.py3-none-any.whl'  # Local path in /tmp directory

# Initialize the S3 file system
fs = s3fs.S3FileSystem()

# Download the .whl file from S3 to the local path
fs.get(s3_path, local_path)

# Install the downloaded .whl file using pip
subprocess.check_call([sys.executable, '-m', 'pip', 'install', local_path])
